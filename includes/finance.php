...
function canManageFinanceRecords(): bool
{
    return in_array(currentRole(), ['super_admin', 'finance', 'registrar', 'hod'], true);
}

function requireFinanceManagement(): void
{
    requireLogin();
    if (!canManageFinanceRecords()) {
        flash('danger', 'You do not have permission to manage financial records.');
        redirect(url('dashboard.php'));
    }
}

function addFinanceHold(int $studentId, string $holdType, string $reason, ?int $createdBy = null, bool $autoGenerated = false): int
{
    $db = getDB();
    $stmt = $db->prepare(
    ...
    auditLog('finance_hold_created', 'finance_hold', $holdId);
    return $holdId;
}

function liftFinanceHold(int $holdId, ?int $liftedBy = null): void
{
    $db = getDB();
    $stmt = $db->prepare('SELECT id FROM finance_holds WHERE id = ? AND is_active = 1');
    $stmt->execute([$holdId]);
    if (!$stmt->fetchColumn()) {
        throw new InvalidArgumentException('Financial hold not found.');
    }

    $db->prepare(
        'UPDATE finance_holds SET is_active = 0, lifted_at = NOW(), lifted_by = ? WHERE id = ?'
    )->execute([$liftedBy ?? ($_SESSION['user_id'] ?? null), $holdId]);
    auditLog('finance_hold_lifted', 'finance_hold', $holdId);
}

function refreshInvoiceStatus(int $invoiceId): void
{
    $db = getDB();
    $stmt = $db->prepare('SELECT total_amount, amount_paid, due_date, status FROM invoices WHERE id = ?');
    ...
    $balance = max(0.0, (float) $invoice['total_amount'] - (float) $invoice['amount_paid']);
    $status = 'pending';
    if ((float) $invoice['amount_paid'] > 0.01 && $balance > 0.01) {
        $status = 'partial';
    } elseif ($balance <= 0.01) {
        $status = 'paid';
...
function updateInvoice(int $invoiceId, array $data): void
{
    $db = getDB();
    ...
    if (($invoice['status'] ?? '') === 'cancelled') {
        throw new RuntimeException('Cancelled invoices cannot be updated.');
    }

    $currency = $data['currency'] ?? ($invoice['currency'] ?? 'USD');
    $invoiceType = $data['invoice_type'] ?? ($invoice['invoice_type'] ?? 'invoice');
    $feeStructureId = array_key_exists('fee_structure_id', $data) && $data['fee_structure_id'] !== ''
        ? (int) $data['fee_structure_id']
        : null;
    $sponsorId = array_key_exists('sponsor_id', $data) && $data['sponsor_id'] !== ''
        ? (int) $data['sponsor_id']
        : null;
    ...
    $db->prepare(
        'UPDATE invoices
         SET currency = ?, invoice_type = ?, fee_structure_id = ?, sponsor_id = ?, due_date = ?, notes = ?
         WHERE id = ?'
    )->execute([
        $currency,
        $invoiceType,
    ...
    auditLog('invoice_updated', 'invoice', $invoiceId);
}

function cancelInvoice(int $invoiceId): void
{
    $db = getDB();
    $stmt = $db->prepare('SELECT amount_paid, status FROM invoices WHERE id = ?');
    $stmt->execute([$invoiceId]);
    $invoice = $stmt->fetch();
    if (!$invoice) {
        throw new InvalidArgumentException('Invoice not found');
    ...
    $db->prepare("UPDATE invoices SET status = 'cancelled' WHERE id = ?")->execute([$invoiceId]);
    auditLog('invoice_cancelled', 'invoice', $invoiceId);
}

function voidPayment(int $paymentId): void
{
    $db = getDB();
    ...
    if (($payment['status'] ?? 'confirmed') === 'rejected') {
        return;
    }
    ...
    try {
        if (($payment['status'] ?? 'confirmed') === 'confirmed') {
            $invoiceStmt = $db->prepare('SELECT amount_paid FROM invoices WHERE id = ? FOR UPDATE');
            $invoiceStmt->execute([(int) $payment['invoice_id']]);
            $invoice = $invoiceStmt->fetch();
            if ($invoice) {
                $newPaid = max(0.0, (float) $invoice['amount_paid'] - (float) $payment['amount']);
                $db->prepare('UPDATE invoices SET amount_paid = ? WHERE id = ?')
                    ->execute([$newPaid, (int) $payment['invoice_id']]);
                refreshInvoiceStatus((int) $payment['invoice_id']);
            }
        }
    ...
    auditLog('payment_voided', 'payment', $paymentId);
}
...
function confirmPayment(int $paymentId): void
{
    ...
    auditLog('payment_confirmed', 'payment', $paymentId);
}

function updatePayment(int $paymentId, array $data): void
{
    $db = getDB();
    $db->beginTransaction();
    try {
        $stmt = $db->prepare('SELECT * FROM payments WHERE id = ? FOR UPDATE');
        $stmt->execute([$paymentId]);
        $payment = $stmt->fetch();
        if (!$payment) {
            throw new InvalidArgumentException('Payment not found');
        ...
        $invoiceStmt = $db->prepare('SELECT * FROM invoices WHERE id = ? FOR UPDATE');
        $invoiceStmt->execute([(int) $payment['invoice_id']]);
        $invoice = $invoiceStmt->fetch();
        if (!$invoice) {
            throw new InvalidArgumentException('Invoice not found');
        ...
        $newStatus = $data['status'] ?? $payment['status'] ?? 'confirmed';
        if (!in_array($newStatus, ['pending', 'confirmed', 'rejected'], true)) {
            throw new RuntimeException('Invalid payment status.');
        }

        $oldConfirmed = ($payment['status'] ?? 'confirmed') === 'confirmed';
        $newConfirmed = $newStatus === 'confirmed';
        $currentPaid = (float) $invoice['amount_paid'];

        if ($oldConfirmed) {
            $currentPaid = max(0.0, $currentPaid - (float) $payment['amount']);
        }
        if ($newConfirmed) {
        ...
        $db->prepare(
            'UPDATE payments
             SET amount = ?, currency = ?, exchange_rate = ?, payment_method = ?, reference = ?, pop_file = ?, status = ?, sponsor_id = ?
             WHERE id = ?'
        )->execute([
            $newAmount,
            $data['currency'] ?? $payment['currency'] ?? 'USD',
            $data['exchange_rate'] ?? $payment['exchange_rate'] ?? 1,
            $data['payment_method'] ?? $payment['payment_method'],
            $data['reference'] ?? $payment['reference'],
            array_key_exists('pop_file', $data) ? $data['pop_file'] : ($payment['pop_file'] ?? null),
            $newStatus,
            array_key_exists('sponsor_id', $data) ? $data['sponsor_id'] : ($payment['sponsor_id'] ?? null),
            $paymentId,
        ]);

        if ($oldConfirmed || $newConfirmed) {
            $db->prepare('UPDATE invoices SET amount_paid = ? WHERE id = ?')->execute([$currentPaid, (int) $payment['invoice_id']]);
            refreshInvoiceStatus((int) $payment['invoice_id']);
        }
    ...
    if (($data['status'] ?? ($payment['status'] ?? 'confirmed')) === 'confirmed') {
        liftAutoHoldsForStudent((int) $invoice['student_id']);
    }

    auditLog('payment_updated', 'payment', $paymentId);
}

function getReceiptById(int $paymentId): ?array
{
    $db = getDB();
    $stmt = $db->prepare(
...
function deletePayment(int $paymentId): void
{
    ...
    try {
        ...
        if (($payment['status'] ?? 'confirmed') === 'confirmed') {
            $newPaid = max(0.0, (float) $invoice['amount_paid'] - (float) $payment['amount']);
            $db->prepare('UPDATE invoices SET amount_paid = ? WHERE id = ?')->execute([$newPaid, (int) $payment['invoice_id']]);
            refreshInvoiceStatus((int) $payment['invoice_id']);
        }
    ...
    auditLog('payment_deleted', 'payment', $paymentId);
}
...
function sendPaymentReminder(int $studentId): bool
{
    $db = getDB();
    $stmt = $db->prepare(
        'SELECT COALESCE(s.email, u.email) AS email, COALESCE(s.first_name, up.first_name) AS first_name, COALESCE(s.last_name, up.last_name) AS last_name, s.student_number
         FROM students s
         LEFT JOIN users u ON u.id = s.user_id
         LEFT JOIN user_profiles up ON up.user_id = u.id
         WHERE s.id = ?'
